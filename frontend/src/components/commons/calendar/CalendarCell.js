...
const CalendarCell = ({ index,  week, }) => {
    ...
    return(
    <td  className={`border h-120 hover:shadow-inner ${ index % 7 === 0 || index % 6 === 0 ? 'bg-gray-100':'' } `}
        onContextMenu={
            (event) => {
                setContextMenu({
                    isOpen: true,
    ...
    >
        ...
        >
            ...
            </ul>
            {
                ...
                <div className="calendar-cell-customize-bar absolute bottom-2 left-0 w-full z-50 flex items-center justify-center shadow-lg ">
                    <button
                        className="flex-1 h-5 bg-white border-1 border-gray-400 hover:bg-gray-50"
                        onClick={(e) => changeTdColor(e, days.format('YYYYMMDD'), "white")}
                    />
                    <button
                        className="flex-1 h-5 bg-red-100 border-1 border-gray-400 hover:bg-red-200"
                        onClick={(e) => changeTdColor(e, days.format("YYYYMMDD"), "red")}/>
                    <button
                        className="flex-1 h-5 bg-blue-100 border-1 border-gray-400 hover:bg-blue-200"
                        onClick={(e) => changeTdColor(e, days.format("YYYYMMDD"), "blue")}/>
                    <button
                        className="flex-1 h-5 bg-green-100 border-1 border-gray-400 hover:bg-green-200"
                        onClick={(e) => changeTdColor(e, days.format("YYYYMMDD"), "green")}/>
                    <button
                        className="flex-1 h-5 bg-yellow-100 border-1 border-gray-400 hover:bg-yellow-200"
                        onClick={(e) => changeTdColor(e, days.format("YYYYMMDD"), "yellow")}/>
                    <button
                        className="flex-1 h-5 bg-purple-100 border-1 border-gray-400 hover:bg-purple-200"
                        onClick={(e) => changeTdColor(e, days.format("YYYYMMDD"), "purple")}/>
                    <button
                        className="flex-1 h-5 bg-indigo-100 border-1 border-gray-400 hover:bg-indigo-200"
