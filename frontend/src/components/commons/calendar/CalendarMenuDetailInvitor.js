...
const CalendarMenuDetailInvitor = () => {
  ...
  const submitInviteInput = async () => {
    ...
    formData.append(
      'actionUrl',
      `/users/${inviteInput}/join/calendars/${calendarDetail.id}`,
    )
    formData.append(
      'content',
      `${user.nickname}님이 당신을 ${calendarDetail.name} 그룹에 초대했어요!`,
    )
  ...
  return !calendarDetail.isDefault ? ( // 기본캘린더가 아닐 경우만 초대 보이기
    <>
    ...
    <div className="w-full p-2 mb-2 text-sm bg-white border rounded-md">
      이 캘린더는 개인 캘린더입니다. 팀 공유 캘린더가 필요하다면, 새로운
      캘린더를 만들어주세요!🐥
    </div>
  )
}
