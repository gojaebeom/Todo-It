...
const userUpdateModalEvent = (UserUpdateModal) => {
  return () => {
    ...
    const [updateUserModalOpen, setUpdateUserModalOpen] = useRecoilState(
    ...
    const changeImageEvent = (e) => {
      readImgFile(e, (event, file) => {
        setUserEdit({
          ...userEdit,
          profilePreviewImg: event.target.result,
          profileImgFile: file,
        })
      })
    }
    ...
    const changeInputEvent = (e) => {
      ...
      if (name === 'nickname') setUserEdit({ ...userEdit, nickname: value })
    }
    ...
    const clickDeleteUserEvent = async () => {
      ...
      if (result === '회원탈퇴') {
        ...
        setToast({
          open: true,
          message: '그동안 Todoit을 이용해주셔서 감사합니다.',
          type: 'INFO',
          second: 2000,
        })
    ...
    const submitEvent = async () => {
      ...
      if (userEdit.profileImgFile)
        formData.append('profileImg', userEdit.profileImgFile)
